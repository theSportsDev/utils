...
| 모듈     | 설명                                                                                      | 문서                                       |
| -------- | ----------------------------------------------------------------------------------------- | ------------------------------------------ |
| Logger   | 개인정보 필터·구조화 JSON·요청 추적을 제공하는 Winston 기반 공용 로거입니다. `LoggerFactory.create` 설정은 JavaScript IDE 자동완성을 지원하며, 중앙 CloudWatch/OpenSearch 및 향후 Datadog Firehose 연동을 지원합니다. | [logger/README.md](./src/logger/README.md) |
| Notifier | Slack 에러·서버 로그·스크립트 결과 발송기. `ErrorNotifier`와 `DevNotifier`를 제공합니다. | [notifier/README.md](./src/notifier/README.md) |
